        ...
        <form id="infoForm">
            <div class="mb-3">
                ...
                <div class="input-group">
                    <input type="text" class="form-control" id="student_code" name="student_code" inputmode="numeric"
                        maxlength="3" placeholder="کد دانش آموزی 3 رقمی را وارد کنید">
                </div>
            </div>
...
@section('script')
    ...
    <script>
        ...
        //سابمیت خودکار فرم بعد از وارد کردن کد دانش آموزی
        let submitting = false;

        studentCodeInput.addEventListener('input', () => {
            // حذف فاصله و کاراکترهای غیرعددی
            const value = studentCodeInput.value.replace(/\D/g, '');
            studentCodeInput.value = value;

            if (submitting) return;

            if (value.length === 3) {
                // بررسی صحت فرمت کد دانش آموزی
                if (/^\d{3}$/.test(value)) {
                    // سابمیت خودکار
                    submitting = true;
                    form.requestSubmit();
                } else {
                    showToast('فرمت کد دانش آموزی صحیح نیست', 'warning');
                }
            }
        });
        ...
        form.addEventListener('submit', async (e) => {
                ...
                data.student_code = convertPersianDigits(data.student_code);


                if (!data.student_code) {
                    return showToast('فیلد کد دانش آموزی را کامل کنید', 'danger');
                }

                if (!/^\d{3}$/.test(data.student_code)) {
                    return showToast('فرمت کد دانش آموزی وارد شده صحیح نیست', 'danger');
                }
                try {
                    // ارسال درخواست به سرور
                    const searchUrl = "{{ route('search.student.ByStudentCode') }}"
