    ...
    <div>
        <table id="reportTable" class="display nowrap" style="width: 100%">
            <thead>
                ...
                <th scope="col">میزان تاخیر</th>
                <th scope="col">موجه بودن</th>
                ...
                <tr>
                    ...
                    <th><input type="text" placeholder="وضعیت حضور" /></th>
                    <th><input type="text" placeholder="جستجو میزان تاخیر" /></th>
                    <th><input type="text" placeholder="موجه بودن" /></th>
            ...
            <tbody>
                @foreach ($attendances as $attendance)
                    <tr>
                        <td>{{ $attendance->student->family ?? '—' }}</td>
                        <td>{{ $attendance->student->name ?? '—' }}</td>
                        <td>{{ $attendance->student->schoolClass->name ?? '—' }}</td>
                        <td>{{ toJalali($attendance->date) }}</td>
                        ...
                        @if ($attendance->status == 'present' || $attendance->status == 'unknown')
                            <td> - </td>
                        @else
                            <td>{{ $attendance->is_excused ? 'موجه' : 'غیرموجه' }}</td>
                        @endif

                        <td>{{ $attendance->description }}</td>

                        <td>{{ $attendance->register->name ?? '-' }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
