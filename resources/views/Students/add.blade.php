...
@section('body')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h4 class="fw-bold">دانش آموز جدید</h4>
    </div>
    <div>
        <form action="{{ route('students.store') }}" data-confirm="create" data-confirm-item="دانش آموز" method="post"
            enctype="multipart/form-data">
            @csrf
            <div class="alert alert-info text-center col-12 col-md-9" role="alert">
                اطلاعات اولیه (ضروری)
            </div>
            ...
            <div class="row d-flex">
                <div class="col-3 ">
                    ...
                    <img id="croppedPreview" class="croppedPreview border border-primary"
                        src="{{ old('cropped_image', asset('images/default.png')) }}">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <input id="national_code" name="national_code" placeholder="مثال: 0123456789" type="text"
                            class="form-control" value="{{ old('national_code') }}" />
                        <div class="text text-danger">
                            @error('national_code')
                                {{ $message }}
                            @enderror
                    ...
                    <div class="col-md-4">
                        ...
                        <input id="student_code" name="student_code" type="text" class="form-control"
                            placeholder="مثال: 701" value="{{ old('student_code') }}" />
                        <div class="text text-danger">
                            @error('student_code')
                                {{ $message }}
                            @enderror
            ...
            <div class="col-12 col-md-9">
                ...
                <div id="add-first-phone-wrapper" class="{{ count($phones) ? 'd-none' : '' }}">
                    <button type="button" id="add-first-phone" class="btn btn-outline-success">
                        + افزودن شماره
                    </button>
                </div>
                ...
                <div id="phones-wrapper" class="{{ count($phones) ? '' : 'd-none' }}">
                            ...
                            <div class="card-body">
                                    ...
                                    <div class="col-md-3">
                                        <div class="form-check mt-4">
                                            <input type="checkbox" name="phones[{{ $i }}][is_just_virtual]"
                                                ...
                                                {{ !empty($phone['is_just_virtual']) ? 'checked' : '' }}
                                                {{ $i < count($phones) - 1 ? 'disabled' : '' }}>
                                ...
                                <div class="mt-3">
                                    ...
                                    <textarea name="phones[{{ $i }}][description]" rows="1"
                                        class="form-control description @error("phones.$i.description") is-invalid @enderror"
                                        {{ $i < count($phones) - 1 ? 'readonly' : '' }}>{{ $phone['description'] ?? '' }}</textarea>
                    ...
                    <button type="button" id="add-phone" class="btn btn-outline-success">
                        + افزودن شماره جدید
                    </button>
                </div>
            </div>
    ...
    <script>
        ...
        function validateItem($item) {
            ...
            if (!phoneFor.val()) {
                ...
                valid = false;
            } else phoneFor.removeClass('is-invalid');

            if (!/^09\d{9}$/.test(phoneNum.val())) {
                phoneNum.addClass('is-invalid');
                valid = false;
            } else phoneNum.removeClass('is-invalid');

            if (phoneFor.val() === 'Other') {
                if (!persianRegex.test(desc.val().trim())) {
        ...
        function lockItem($item) {
            $item.find('input[type="text"], textarea').prop('readonly', true);
            $item.find('select, input[type="checkbox"]').each(function() {
                const $el = $(this);
                $('<input>', {
                    type: 'hidden',
                    name: $el.attr('name'),
